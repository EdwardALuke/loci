...
#define MIN_PROBE_FREQ 10
  ...
  void dynamic_schedule_rule::iterative_weighted_static () {
    ...
    MPI_Allreduce(&exec_time, &max_time, 1, MPI_FLOAT, MPI_MAX,
                  facts1->get_comm()) ;
    double ef = total_time/(max_time*float(facts1->get_comm_size())) ;

    if(facts1->get_comm_rank() == 0) 
	...
	time += chunkData[i].chunkTime[0]+chunkData[i].chunkTime[1] ;
      ...
      MPI_Allreduce(&time, &total_time, 1, MPI_FLOAT, MPI_SUM,
                    facts1->get_comm()) ;
      float mean_time = total_time/float(facts1->get_comm_size()) ;
...
#define SELECT_GIVE
	...
	for(size_t i=0;i<send_list.size();++i)
      ...
      vector<float> time_xfers(facts1->get_comm_size()) ;
	...
	debugout << "time xfers =";
	for(int i=0;i<facts1->get_comm_size();++i)
	...
	for(int j=send_chunks.size()-1;j>=0;--j) {
	  if(send_chunks[j].second >=0) {
	    if(ptime - send_chunks[j].first > -mean_time*eff_tol) {
	      ...
	      if(send_chunks[j].second == facts1->get_comm_rank()) {
#ifdef VERBOSE
	...
	vector<int> chunk_groups(facts1->get_comm_size()) ;
	...
	vector<int> chunk_displ(facts1->get_comm_size()) ;
	chunk_displ[0] = 0 ;
	for(int i=1;i<facts1->get_comm_size();++i) 
	  chunk_displ[i] = chunk_displ[i-1]+chunk_groups[i-1] ;

	int ntchunks = (chunk_displ[facts1->get_comm_size()-1]+
			chunk_groups[facts1->get_comm_size()-1]) ;
	...
	for(int i=facts1->get_comm_size()-1;i>=0;--i) 
	  if(time_xfers[i] < 0) {
	    vector<int> sendto_p ;
	...
	local_send_list.push_back(tmp) ;
      ...
      vector<int> sendSizes(facts1->get_comm_size(),0) ;
      for(size_t i=0;i<sendChunks.size();++i)
	sendSizes[sendChunks[i].proc] = sendChunks[i].chunkList.size() ;
      
      vector<int> recvSizes(facts1->get_comm_size(),0) ;
	...
	for(size_t i=0;i<recvChunks.size();++i)
      ...
      if(facts1->get_comm_rank() == 0) {
        debugout << "IWS Schedule: Each processor communicating with an average of "
                 << double(nsendrecvs)/double(facts1->get_comm_size())
                 << " processors." << endl
