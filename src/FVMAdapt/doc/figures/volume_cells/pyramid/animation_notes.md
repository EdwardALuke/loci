...
`pyramid_face_based_isotropic.tex` draws the same cell with the face-centroid,
edge-midpoint, and volume-centroid split lines.

`pyramid_face_based_isotropic_exploded.tex` defines the wireframe exploded
...
```sh
bash src/FVMAdapt/doc/figures/volume_cells/pyramid/render_pyramid_face_based_isotropic_exploded_motion_yaw_wire.sh
```

The combined wire motion/yaw script regenerates:

- `pyramid_face_based_isotropic_exploded_motion_yaw_wire.gif`
