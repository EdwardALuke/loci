...
`hexahedron_face_based_isotropic.tex` draws the same cell with the
face-centroid, edge-midpoint, and volume-centroid split lines.

`hexahedron_face_based_isotropic_exploded.tex` defines the wireframe exploded
...
```sh
bash src/FVMAdapt/doc/figures/volume_cells/hexahedron/render_hexahedron_face_based_isotropic_exploded_motion_yaw_wire.sh
```

The combined wire motion/yaw script regenerates:

- `hexahedron_face_based_isotropic_exploded_motion_yaw_wire.gif`
