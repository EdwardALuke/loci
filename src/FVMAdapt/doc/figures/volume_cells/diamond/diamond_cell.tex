...
\begin{tikzpicture}[
  ...
  \draw[axis] (Oaxis) -- (-1.20,-0.42,1.18) node[above,font=\scriptsize] {$\zeta$};
  ...
  % Original vertices are black, with the back equator vertex softened.
  \foreach \p in {L,F,R,T,S} {
    \fill[black] (\p) circle[radius=0.75mm];
  }
  \fill[black!65] (B) circle[radius=0.75mm];
\end{tikzpicture}
\end{document}
