...
tex_file="$script_dir/tetrahedron_face_based_isotropic_exploded.tex"
base_name=tetrahedron_face_based_isotropic_exploded_motion_yaw_wire
out_gif="$script_dir/${base_name}.gif"

work_dir=$(mktemp -d "${TMPDIR:-/tmp}/fvmadapt-tetrahedron-face-motion-yaw-wire.XXXXXX")
...
render_frame() {
  ...
  pdflatex -halt-on-error -interaction=nonstopmode \
    ...
    "\\def\\fvmadaptExplodeAmount{$amount}\\def\\fvmadaptExplodeScale{1.56}\\def\\fvmadaptYawAngle{$yaw}\\input{$tex_file}" >/dev/null
