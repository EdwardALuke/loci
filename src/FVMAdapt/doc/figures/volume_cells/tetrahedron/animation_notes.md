...
`tetrahedron_face_based_isotropic.tex` draws the same cell with the
face-centroid, edge-midpoint, and volume-centroid split lines.

`tetrahedron_face_based_isotropic_exploded.tex` defines the wireframe exploded
...
```sh
bash src/FVMAdapt/doc/figures/volume_cells/tetrahedron/render_tetrahedron_face_based_isotropic_exploded_motion_yaw_wire.sh
```

The combined wire motion/yaw script regenerates:

- `tetrahedron_face_based_isotropic_exploded_motion_yaw_wire.gif`
