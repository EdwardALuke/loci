...
`prism_cell.tex` draws the prism cell, and `prism_face_based_isotropic.tex`
draws the same cell with the face-centroid, edge-midpoint, and volume-centroid
split lines.

`prism_face_based_isotropic_exploded.tex` defines the wireframe exploded prism
...
```sh
bash src/FVMAdapt/doc/figures/volume_cells/prism/render_prism_face_based_isotropic_exploded_motion_yaw_wire.sh
```

The combined wire motion/yaw script regenerates:

- `prism_face_based_isotropic_exploded_motion_yaw_wire.gif`
