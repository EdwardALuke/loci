...
\begin{tikzpicture}[
  ...
  \draw[axis] (Oaxis) -- (1.85,-0.42,-0.25) node[below right,font=\scriptsize] {$\xi$};
  \draw[axis] (Oaxis) -- (-1.18,1.18,-0.25) node[above left,font=\scriptsize] {$\eta$};
  \draw[axis] (Oaxis) -- (-1.18,-0.42,1.18) node[above,font=\scriptsize] {$\zeta$};
  ...
  \draw[hidden] (D) -- (C);
  \draw[hidden] (H) -- (Gxi);
  \fill[black!65] (Oeta) circle[radius=0.72mm];
  \fill[black!65] (D) circle[radius=0.72mm];
  \fill[black!65] (H) circle[radius=0.72mm];
  ...
  \foreach \p in {Oxi,Ozeta,B,C,E,F,Gxi,Geta,Gzeta} {
    \fill[black] (\p) circle[radius=0.72mm];
  }
