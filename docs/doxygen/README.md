...
- `pdflatex` with TikZ/PGF, `mutool`, and ImageMagick (`magick` or `convert`)
  for generated FVMAdapt motion GIFs referenced by the Doxygen pages. The
  Doxygen asset staging step checks these utilities before rendering GIFs and
  reports the missing packages if they are unavailable.
...
Generated FVMAdapt motion GIF intermediates are written under
