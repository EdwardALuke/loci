...
# Some cell-type datasets are absent when the mesh has no cells of that type.
element_count()
{
  dataset=$1
  count=$(
    "${H5DUMP:-h5dump}" -H -d "/elements/$dataset" "$topology" 2>/dev/null |
...
# Extra coordinate planes show which directions acquired refined nodes.
coordinate_planes()
{
  column=$1
...
cells=$(
  "${H5DUMP:-h5dump}" -a "/file_info/numCells" "$mesh" |
...
sh "$script_dir/../mesh_summary.sh" "$mesh" "$check_log"
